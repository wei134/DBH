...
public:
	...
	void Append( const TYPE & item ){
		size_t n = this->size();
		if( n + 1 >= this->capacity() ) this->reserve( n + n/5 + 1 ); //vector 的reserve增加了vector的capacity，但是它的size没有改变！
		this->push_back( item );
	}

	void Replace(const TYPE & item,int eee){//替换某个元素为& item
		//replace (0,this->size(), item);
		//vector <int>::iterator Iter;
		//Iter = this.begin( ) ;
		//Iter++;
		if (this->size()==eee){
			this->erase(this->begin()+eee-1);
			this->push_back( item );
		}else {
			this->erase(this->begin( )+eee-1);
			this->insert(this->begin( )+eee-1,item);
		}
...
extern Vector<Vector<int> > clusters1;
struct IndexCount
{
	int index;
	int count;
...
/*struct represitive_seq{//纠错后的代表序列
	char *data;
...
public:
	...
	Vector<Sequence*>  sequences;
	Vector<int>        rep_seqs;
	Vector<string> represitive;    //纠错后的代表序列
	Vector< Vector<int> > correct_is;        //纠错后的代表序列属于初始聚类哪一类
	Vector<int> correct_id;                //纠错后的代表序列属于初始聚类哪一类的 标签

	Vector< Vector<int> > last;       //最终类
	Vector< Vector<int> > SL_last;    //代表序列聚类后的标签
	Vector<int> remain_id;          //剩余序列标签
	Vector<int> label;                //最后标签
	...
	Vector<Vector<int> > WriteExtra1D1( const Options & options );
	void WriteExtra1D2( const Options & options );
	void WriteCorrectedSeq( const Options & options );
	void Write_SL_correctSeq(Vector< Vector<string> > vector1,const Options & options);
	...
	void correct_precluster(const Options & options,Vector< Vector<int> > clusters,int max_len);
	Vector< Vector<string> > SL(const Options & options,Vector<Vector<int> > clusters);
	void remain(Vector<Vector<int> >pre,Vector<Vector<string> > SL_correctSeq,const Options &options);
	Vector<Vector<int> >  precluster_vector(const Options & options);//只返回初始聚类的标签
	void SwapIn( int seg, bool reponly=false );
	void SwapOut( int seg );

	//void Sort( int first, int last );
	int SortDivide( Options & options, bool sort=true );
	...
	void ClusterOne( Sequence *seq, int id, WordTable & table,WordTable & table2,WordTable & table3,
		WorkingParam & param,WorkingParam & param2,WorkingParam & param3, WorkingBuffer & buf,WorkingBuffer & buf2,WorkingBuffer & buf3, const Options & options );
	...
	void DoClustering1( const Options & options );
	...
	int  CheckOne( Sequence *seq, WordTable & tab,WordTable & tab2,WordTable & tab3, WorkingParam & par,WorkingParam & par2,WorkingParam & par3, WorkingBuffer & buf,WorkingBuffer & buf2,WorkingBuffer & buf3, const Options & opt );
	...
	int  CheckOneAA( Sequence *seq, WordTable & tab, WordTable & tab2,WordTable & tab3,WorkingParam & par, WorkingParam & par2,WorkingParam & par3,WorkingBuffer & buf, WorkingBuffer & buf2,WorkingBuffer & buf3,const Options & opt );
};
