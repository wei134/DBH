...
Vector<Vector<int> > clusters1( 100000 );
...
bool Options::SetOptionCommon( const char *flag, const char *value )
{
	...
	else if (strcmp(flag, "-k" ) == 0) kmer_num = atol(value),useKmerNum=true;
	//else if (strcmp(flag, "-M" ) == 0) max_memory  = atoll(value) * 1000000;
	else if (strcmp(flag, "-M" ) == 0) max_memory  = atol(value) * 1000000;
	//else if (strcmp(flag, "-e" ) == 0) error_rate  =1.0 - 2.0*atof(value),useIdentity = true;
		...
		if( threads != intval ) printf( "Number of CPUs to be used is: %i\n\n", threads );
#else
...
bool Options::SetOptions( int argc, char *argv[], bool twod, bool est )
{
	...
	//printf( ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n" );
	////printf( "Program: dbClust, V" WITH_OPENMP ", %s, " __TIME__ "\n", date );
	//printf( "Command:" );
	n = 9;
	for(i=0; i<argc; i++){
		//printf( "\n         %s", argv[i] );
...
int print_usage (char *arg){
	cout<<"	\nUsage: ./db -i input file, -o output file name, -c cluster cutoff, -k number of kmer (default 10), -T CPU number (default 10)"<<endl;
	cout<<"	\nExample: ./db -i seq.fastq -o db97 -c 0.97"<<endl<<endl;
	return 0;
...
{
	if( useIdentity && useDistance ) bomb_error( "can not use both identity cutoff and distance cutoff" );
	if( useDistance ){
	...
	if (! isEST && (tolerance < 0 || tolerance > 5) ) bomb_error("invalid tolerance");
...
*/
int seq_align(const string &a, const string &b, int alpha_gap, 
        ...
        string &b_aligned)
{
    ...
    for (size_t i = 1; i <= n; ++i)
    {
        for (size_t j = 1; j <= m; ++j)
        {
            char x_i = a[i-1];
            char y_j = b[j-1];
            A[i][j] = min2(A[i-1][j-1] + alpha[x_i - 'A'][y_j - 'A'],
                          A[i-1][j] + alpha_gap,
                          A[i][j-1] + alpha_gap);
        }
    }
	...
	/*for (int gang=0;gang<a_aligned.length();gang++){
		if (a_aligned[gang]==b_aligned[gang]){
			overlap++;
		}
	}
	*/
...
void Sequence::PrintInfo( int id, FILE *fin, FILE *fout,FILE *fout2, const Options & options, char *buf )
{
	const char *tag = options.isEST ? "nt" : "aa";
	bool print = options.print != 0;
	bool strand = options.isEST;
	fprintf( fout, "%i\t%i %i%s, >%s...", id,index+1, size, tag,identifier+1 );
	//fprintf( fout2, "%i ", index+1 );
	if( identity ){
		int *c = coverage;
		fprintf( fout, " at " );
...
#endif
int SequenceDB::SortDivide( Options & options, bool sort )
{
	int i, j, k, len;
	...
	for (i=0; i<N; i++) {
		...
		//seq->data2=seq->data;
		//printf("第%d个seq data是%s\n",i,sequences[i]->data);
		//printf("第%d个seq data2是%s\n",i,sequences[i]->data2);
	...
	//cout << "Longest: "<< max_len << " and shortest: "<< min_len << endl;
...
Vector<Vector<int> > SequenceDB::WriteExtra1D1( const Options & options )
{
	string aaa1=options.output;
	char aaa2[25];
	int iii;
	for ( iii=0;iii<aaa1.length();iii++){
	aaa2[iii]=aaa1[iii];	
	}
	//aaa2=&aaa1;
	aaa2[iii]='\0';
	string db_clstr = strcat(aaa2 , ".clstr");

	//david
	string db_clstr2 = strcat(aaa2 , ".clstr_index");

	string db_clstr_bak = strcat(aaa2, ".bak.clstr");
	int i, k, N = sequences.size();
	vector<long long> sorting( N );
	for (i=0; i<N; i++) sorting[i] = ((long long)sequences[i]->index << 32) | i;
	std::sort( sorting.begin(), sorting.end() );

	FILE *fout,*fout2, *fin = fopen( options.input.c_str(), "r" );
	...
	if( options.backupFile ){
		...
		for (i=0; i<N; i++) {
			Sequence *seq = sequences[ sorting[i] & 0xffffffff ];
			seq->PrintInfo( seq->cluster_id, fin, fout,fout2, options, buf );
		}
		fclose( fout );
	}
	...
	//david
	fout2 = fopen( db_clstr2.c_str(), "w+" );
	for (i=0; i<M; i++) {
		fprintf( fout, ">Cluster %i\n", i );
		for (k=0; k<(int)clusters[i].size(); k++){
			sequences[ clusters[i][k] ]->PrintInfo( k, fin, fout,fout2, options, buf );
			fprintf( fout2, "%i ",sequences[ clusters[i][k] ]->index+1 );
		}
		//cout<<"i="<<i<<endl;
		fprintf( fout2, "\n" );
	}
...
void SequenceDB::WriteExtra1D2( const Options & options )
{
	string aaa1=options.output;
	char aaa2[35];
	int iii;
...
void SequenceDB::Write_SL_correctSeq(Vector<Vector<string> > vector1,const Options & options){
	string qq1=options.output;
	char qq2[35],qq3[35],qq4[35];
	int iii;
	...
	qq3[iii]='\0';
	qq4[iii]='\0';
	string corrected_sequence = strcat(qq2 , ".rep.sl");
	string final_id = strcat(qq3 , ".final");
	string final_label = strcat(qq4 , ".label");

	FILE *representive_out= fopen( corrected_sequence.c_str(), "w+" );
	FILE *finallll= fopen( final_id.c_str(), "w+" );
	FILE *fina_labellll= fopen( final_label.c_str(), "w+" );
	if (representive_out==NULL){
		printf("file can not open!\n" );
	}
	//fprintf( representive_out, ">Cluster \n" );
	for (int yy=0;yy<vector1.size();yy++){
		//string cdcd=;
		fprintf( representive_out, ">cluster%i\n",yy+1);
		//fprintf( finallll, ">cluster%i\n",yy+1);
		for (int yyy=0;yyy<vector1[yy].size();yyy++){
			char *rrr=const_cast<char*>(vector1[yy][yyy].c_str());
			fprintf( representive_out, "%s\n", rrr );
		}
		fprintf( representive_out, "\n");
	}
	for (int yy=0;yy<last.size();yy++){
		//string cdcd=;
		//fprintf( representive_out, ">cluster%i\n",yy+1);
		fprintf( finallll, ">cluster%i\n",yy+1);
		for (int yyy=0;yyy<last[yy].size();yyy++){
			label[last[yy][yyy]]=yy;
			//char *rrr=const_cast<char*>(vector1[yy][yyy].c_str());
			fprintf( finallll, "%i\n", last[yy][yyy]+1 );
		}
		fprintf( finallll, "\n");
	}
	for (int ww=0; ww<label.size();ww++){
		fprintf( fina_labellll, "%i\n", label[ww] );
	}
	fclose(representive_out);
	fclose(finallll);
	fclose(fina_labellll);

}



void SequenceDB::WriteCorrectedSeq( const Options & options ){
	string qq1=options.output;
	char qq2[35];
	char qq3[35];
	char qq4[35];
	...
	//aaa2=&aaa1;
	qq2[iii]='\0';qq3[iii]='\0';qq4[iii]='\0';
	//string corrected_sequence = strcat(qq2 , ".representive");
	string all_seqs_after_db = strcat(qq3 , ".after_db");
	//string original = strcat(qq4 , ".ori.fa");

	//FILE *representive_out= fopen( corrected_sequence.c_str(), "w+" );
	FILE *after_out= fopen( all_seqs_after_db.c_str(), "w+" );
	//FILE *original_out= fopen( original.c_str(), "w+" );
	//if (representive_out==NULL){
	//	printf("file can not open!\n" );
	//}
	string::size_type position;
	for (int yy=0;yy<sequences.size();yy++){
		//string cdcd=;
		
		if (sequences[yy]->is_corrected){
			string strBuff=represitive[sequences[yy]->representive_id];
			while((position=strBuff.find("-"))!=string::npos)
			{
				//position=s.find_first_of(flag,position);
				//cout<<position<<" : ";
				strBuff=strBuff.erase(position,1);

			}

			fprintf( after_out, ">%i*\n", yy+1 );

			char *rrr=const_cast<char*>(strBuff.c_str());
			fprintf( after_out, "%s\n", rrr );

			//fprintf( after_out, ">%i*\n", yy+1 );
			//char *rrr=const_cast<char*>(represitive[sequences[yy]->representive_id].c_str());
			//fprintf( after_out, "%s\n", rrr );
		} 
		else{
			fprintf( after_out, ">%i\n", yy+1 );
...
Vector<Vector<int> >  SequenceDB::precluster_vector(const Options & options){//只返回初始聚类的标签

	int N = sequences.size();//
	int num_precluster=0;// 大于等于3的preclude
	vector<long long> sorting( N );
	for (int i=0; i<N; i++) sorting[i] = ((long long)sequences[i]->index << 32) | i;
	std::sort( sorting.begin(), sorting.end() );
...
void SequenceDB::remain(Vector<Vector<int> >pre,Vector<Vector<string> > SL_correctSeq,const Options &options){

	int alpha[alphabets][alphabets];
	int max_id,max_dis=0;
	string bidui1,bidui2;
	float cutoff=options.cluster_thd;
	...
	for (int dandan=0;dandan<remain_id.size();dandan++){
		
		int flagggggggggg=0;
		for (int hh=0;hh<SL_correctSeq.size();hh++){

			int penalty = seq_align((string)sequences[remain_id[dandan]]->data,SL_correctSeq[hh][0] , 2, alpha, bidui1, bidui2);
			float dis=(float)penalty/(float)min(bidui1.length(),bidui2.length());

			if (dis>=cutoff){
				last[hh].Append(remain_id[dandan]);
				flagggggggggg=1;
				break;
		...
		if (flagggggggggg==0){
			last[max_id].Append(remain_id[dandan]);
		}
...
Vector<Vector<string> > SequenceDB::SL(const Options & options,Vector<Vector<int> > pre){
	int alpha[alphabets][alphabets];
	string bidui1,bidui2;
	...
	first.Append(represitive[0]);
	final.Append(first);
	xihuan.Append(correct_id[0]);
	SL_last.Append(xihuan);
	int ff;
	float dis_cutoff=options.cluster_thd;
	for (int dandan=1;dandan<represitive.size();dandan++){
		ff=1;
		for (int ganggang=0;ganggang<final.size();ganggang++){
			for (int bao=0;bao<final[ganggang].size();bao++){
				int penalty = seq_align(represitive[dandan],final[ganggang][bao] , 2, alpha, bidui1, bidui2);
				float dis=(float)penalty/(float)min(bidui1.length(),bidui2.length());
			...
			if (ff==1){
				final[ganggang].Append(represitive[dandan]);
				SL_last[ganggang].Append(correct_id[dandan]);
				break;
			}
		}
		if (ff==0){
			...
			Vector<int> aini;
			aini.Append(correct_id[dandan]);
			first2.Append(represitive[dandan]);
			final.Append(first2);
			SL_last.Append(aini);
			ff=1;
			//break;			
	...
	for (int yy=0;yy<SL_last.size();yy++){
		last.Append(pre[SL_last[yy][0]]);
		for (int zz=1;zz<SL_last[yy].size();zz++){
			for (int ee=0;ee<pre[SL_last[yy][zz]].size();ee++){
				last[yy].Append(pre[SL_last[yy][zz]][ee]);
			}
...
void SequenceDB::correct_precluster(const Options & options,Vector<Vector<int> > clusters,int max_len2 ){
	...
	int max_len=3000;
	int maxxxxxxxxx=10;
	int kmer_num=options.kmer_num;
	int **db=(int **) malloc(maxxxxxxxxx*sizeof(int*));
	for(int gg=0;gg<maxxxxxxxxx;gg++){  
		db[gg]=(int *)malloc((max_len-kmer_num+1)*sizeof(int));
		//memset(db[gg],0,max_length-kmer_num+1);
	}
	
	for(int v=0; v<clusters.size(); v++){
		printf("correct %d group\n",v);
		if (clusters[v].size()>=3){
			Vector<int> pre=clusters[v];
			int maxx_number=pre[0];
			int max_length=sequences[pre[0]]->size;
			
			for (int qqq=1;qqq<min(maxxxxxxxxx,pre.size());qqq++){
				if (max_length<sequences[pre[qqq]]->size){
					maxx_number=pre[qqq];
					max_length=sequences[pre[qqq]]->size;

				}
			}

			for (int xixi=0;xixi<maxxxxxxxxx;xixi++){
				for (int hehe=0;hehe<max_len-kmer_num+1;hehe++){
					db[xixi][hehe]=0;
				}
			}
				...
				int flagg=1;
				string a2, b2;
			    //aligned.Append((string)sequences[pre[1]]->data);
			    for (int vv=0;vv<min(maxxxxxxxxx,pre.size());vv++){
					printf("   correct %d \n",vv);
					string b=(string)sequences[pre[vv]]->data;
					string max_length_sequence=(string)sequences[maxx_number]->data;
					if (b.length()==max_length){
						a2=max_length_sequence;
						b2=b;
					}else{
						int penalty = seq_align(max_length_sequence, b, 2, alpha, a2, b2);
					}
									
					//char *seq_data=sequences[pre[vv]]->data;
				//printf("第%d个seq是\n",pre[vv]+1);
				//strcpy(sss, ss);
					//printf("第%d个seq是%s\n",pre[vv]+1,seq_data);
					//cout<<"第1个seq是"<<ss<<endl;
					if (flagg==1){
						aligned.Append(a2);
						//aligned.Append(b2);
						//printf("%s\n",a2);
					...
					//printf("%s\n",b2);
					for (int d=0;d<min(a2.length(),max_len)-kmer_num+1;d++){
						int flagggg=0;
						for (int dd=0;dd<=aligned.size()-1;dd++){
							if (d<aligned[dd].length()-kmer_num+1 && aligned[dd].substr(d,kmer_num)==b2.substr(d,kmer_num)){
								//printf("equal,d=%i\n",d);
								
								//string gege1=aligned[dd].substr(d,kmer_num);
		; 						//cout<<aligned[dd].substr(d,kmer_num)<<endl;
								//cout<<b2.substr(d,kmer_num)<<endl;
								//cout<<"row="<<dd<<" col="<<d;
								//printf("%s\n",b2.substr(d,kmer_num));
								db[dd][d]++;
								//printf(" db[%i][%i]=%i\n",dd,d,db[dd][d]);
								flagggg=1;
								break;
						...
						/*if (flagggg==0){
							printf("not equal,d=%i\n",d);
							cout<<aligned[aligned.size()-1].substr(d,kmer_num)<<endl;
							cout<<b2.substr(d,kmer_num)<<endl;
							//printf("s%\n",aligned[aligned.size()-1].substr(d,kmer_num));
							//printf("s%\n",b2.substr(d,kmer_num));
							db[aligned.size()-1][d]++;
						}*/
				...
				//找DB路线
				int min_lenth=aligned[0].length();
				for (int sant=1;sant<aligned.size();sant++){
					if (min_lenth>aligned[sant].length())
						min_lenth=aligned[sant].length();
				}
				...
				string db_string;
				printf("    find DB\n");
				for (int cc=0; cc<min_lenth-kmer_num+1; cc++){
					int max_lie=0;
					int which_row;
					for (int qq=0; qq<min(maxxxxxxxxx,pre.size()); qq++){
						if (db[qq][cc]>max_lie){
							max_lie=db[qq][cc];
							which_row=qq;
						}
					...
					//cout<<"row="<<which_row<<" col="<<cc<<" max="<<max_lie<<endl;
					string kmererer=aligned[which_row].substr(cc,kmer_num);
					if (cc==0){
						db_string=kmererer;
					}else db_string+=kmererer.substr(kmer_num-1,1);
					
				}
				//Sequence xiaodan;
				//char *rrr=const_cast<char*>(db_string.c_str());
				represitive.Append(db_string);
				correct_id.Append(v);
				correct_is.Append(clusters[v]);
				
				//printf("第1个seq是s%\n",db_string );
				
				for (int ggg=0; ggg<pre.size();ggg++){
					sequences[pre[ggg]]->representive_id=represitive.size()-1;
					sequences[pre[ggg]]->is_corrected=true;
				}
					...
					represitive.Append((string)sequences[clusters[v][0]]->data);
					correct_id.Append(v);
					correct_is.Append(clusters[v]);

					//printf("第1个seq是s%\n",db_string );

					for (int ggg=0; ggg<clusters[v].size();ggg++){
						sequences[clusters[v][ggg]]->representive_id=represitive.size()-1;
						sequences[clusters[v][ggg]]->is_corrected=true;
					}

					/*

					for (int bb=0;bb<clusters[v].size();bb++){
						remain_id.Append(clusters[v][bb]);
						sequences[clusters[v][bb]]->is_corrected=false;
					}
					//clusters[v].size()>=3
...
void SequenceDB::ClusterOne( Sequence *seq, int id, WordTable & table,WordTable & table2,WordTable & table3,
		WorkingParam & param,WorkingParam & param2,WorkingParam & param3, WorkingBuffer & buffer,WorkingBuffer & buffer2,WorkingBuffer & buffer3, const Options & options )
{
	...
	param2.len_upper_bound = len_bound;
	param3.len_upper_bound = len_bound;
	int flag = CheckOne( seq, table, table2,table3,param,param2,param3, buffer,buffer2,buffer3, options );
	...
	if( flag == 0 ){
		if ((seq->identity>0) && (options.cluster_best)) {
			...
			int aan_no = len - NAA + 1;
			int size = rep_seqs.size();//代表类的序列集合
			rep_seqs.Append( id );
			seq->cluster_id = size;//序列所属类别
			//int id = sequences[k]->cluster_id;
			clusters1[size].Append( id );
			...
			if (frag_size){ /* not used for EST */
				...
				table2.AddWordCounts(aan_no, buffer2.word_encodes, buffer3.word_encodes_no, table2.sequences.size(), options.isEST);
				table3.AddWordCounts(aan_no, buffer2.word_encodes, buffer3.word_encodes_no, table3.sequences.size(), options.isEST);
			}
			table.sequences.Append( seq );
			table2.sequences.Append( seq );
			table3.sequences.Append( seq );
			if( frag_size ){
		...
		clusters1[seq->cluster_id].Append( id );
		if(clusters1[seq->cluster_id].size()==2){
			//int *pp=pp1;
			//int pp=2;
			table2.sequences.Replace(seq,seq->cluster_id+1);
						
		}
			
		else if (clusters1[seq->cluster_id].size()==3){
			//int pp2=3;
			table3.sequences.Replace(seq,seq->cluster_id+1);
		}
...
{
	double scale = 0.5/threads + 0.5/sqrt((double)threads);
...
void SequenceDB::DoClustering( int T, const Options & options )
{
	...
	for(i=0; i<N; ){
		...
		//printf( "m = %i  %i,  %i\n", i, m, m-i );
		//printf( "\r## pregrouping sequences from  %9i  to  %9i#\n", i, m );
		if( last_table.size ){
			int print = (m-i)/20 + 1;
				...
				if( ! stop ){
					...
					if( j == (m0-1) ){ // use m0 to avoid other iterations satisfying the condition:
						...
						for(int ks=i; ks<m; ks++){
							...
							if ( options.store_disk && (seq->state & IS_REDUNDANT) ) seq->SwapOut();
							if( may_stop && word_table.sequences.size() >= 100 ) break;
...
int SequenceDB::CheckOne( Sequence *seq, WordTable & table,WordTable & table2,WordTable & table3, WorkingParam & param,WorkingParam & param2,WorkingParam & param3, WorkingBuffer & buf,WorkingBuffer & buf2,WorkingBuffer & buf3, const Options & options )
{
	int len = seq->size;
	param.len_upper_bound = upper_bound_length_rep(len, options);
	param2.len_upper_bound = upper_bound_length_rep(len, options);
	param3.len_upper_bound = upper_bound_length_rep(len, options);
	if( options.isEST ) return CheckOneEST( seq, table, param, buf, options );
	return CheckOneAA( seq, table,table2, table3, param, param2,param3, buf,buf2,buf3, options );
}
int SequenceDB::CheckOneAA( Sequence *seq, WordTable & table, WordTable & table2,WordTable & table3,WorkingParam & param, WorkingParam & param2,WorkingParam & param3,WorkingBuffer & buf,WorkingBuffer & buf2,WorkingBuffer & buf3, const Options & options )
{
	NVector<IndexCount> & lookCounts = buf.lookCounts;
	NVector<IndexCount> & lookCounts2 = buf2.lookCounts;
	NVector<IndexCount> & lookCounts3 = buf3.lookCounts;
	NVector<uint32_t> & indexMapping = buf.indexMapping;
	NVector<uint32_t> & indexMapping2 = buf2.indexMapping;
	NVector<uint32_t> & indexMapping3 = buf3.indexMapping;
	Vector<INTs> & word_encodes_no = buf.word_encodes_no;
	Vector<INTs> & word_encodes_no2 = buf2.word_encodes_no;
	Vector<INTs> & word_encodes_no3 = buf3.word_encodes_no;
	Vector<INTs> & aap_list = buf.aap_list;
	Vector<INTs> & aap_begin = buf.aap_begin;
	Vector<int>  & word_encodes = buf.word_encodes;
	Vector<int>  & word_encodes2 = buf2.word_encodes;
	Vector<int>  & word_encodes3 = buf3.word_encodes;
	...
	int & required_aa12 = param2.required_aa1;
	int & required_aa13 = param3.required_aa1;
	int & required_aa2 = param.required_aas;
	///////////////////////////////////////////////////////
	int & required_aan = param.required_aan;
	int & required_aan2 = param2.required_aan;
	int & required_aan3 = param3.required_aan;
	...
	param2.ControlShortCoverage( len, options );
	param3.ControlShortCoverage( len, options );
	param.ComputeRequiredBases( options.NAA, 2, options );
	param2.ComputeRequiredBases( options.NAA, 2, options );
	param3.ComputeRequiredBases( options.NAA, 2, options );

	buf.EncodeWords( seq, options.NAA, false );
	buf2.EncodeWords( seq, options.NAA, false );
	buf3.EncodeWords( seq, options.NAA, false );
	...
	//   自己改的
	table2.CountWords(aan_no, word_encodes2, word_encodes_no2, lookCounts2, indexMapping2, false, required_aan2);
	table3.CountWords(aan_no, word_encodes3, word_encodes_no3, lookCounts3, indexMapping3, false, required_aan3);
	...
	int band_left2, band_right2, best_score2, band_width12, best_sum2, len22, alnln2, len_eff12;
	int band_left3, band_right3, best_score3, band_width13, best_sum3, len23, alnln3, len_eff13;
	/////////////////////////////////////////
	int tiden_no, band_center;
	int tiden_no2, band_center2;
	int tiden_no3, band_center3;

	float tiden_pc, distance=0;
	float tiden_pc2, distance2=0;
	float tiden_pc3, distance3=0;
	...
	IndexCount *ic2 = lookCounts2.items;
	ic2 = lookCounts2.items;

	IndexCount *ic3 = lookCounts3.items;
	ic3 = lookCounts3.items;
	/////////////////////////////////////////////////////////////
	for(; ic->count; ic++){
		if( ! frag_size ){
		...
		Sequence *rep2 = table2.sequences[ ic->index ];
		Sequence *rep3 = table3.sequences[ ic->index ];
		len2 = rep->size;
		len22 = rep2->size;
		len23 = rep3->size;
		...
		param2.ControlLongCoverage( len22, options );
		param3.ControlLongCoverage( len23, options );

		if ( has_aa2 == 0 )  { // calculate AAP array
			buf.ComputeAAP( seqi, seq->size );
			buf2.ComputeAAP( seqi, seq->size );
			buf3.ComputeAAP( seqi, seq->size );
			has_aa2 = 1;
		}
		...
		band_width12 = (options.band_width < len+len22-2 ) ? options.band_width : len+len22-2;
		band_width13 = (options.band_width < len+len23-2 ) ? options.band_width : len+len23-2;
		//////////////////////////////////////////////////////////////////////////////////////////////
		diag_test_aapn(NAA1, seqj, len, len2, buf, best_sum,
				band_width1, band_left, band_center, band_right, required_aa1);

		diag_test_aapn(NAA1, seqj2, len, len22, buf2, best_sum2,
			    band_width12, band_left2, band_center2, band_right2, required_aa12);

		diag_test_aapn(NAA1, seqj3, len, len23, buf3, best_sum3,
			    band_width13, band_left3, band_center3, band_right3, required_aa13);
		...
		if (options.print || aln_cover_flag) {//return overlap region
		    ...
		    rc2 = local_band_align(seqi, seqj2, len, len22, mat,
			        best_score2, tiden_no2, alnln2, distance2, talign_info2, 
			        band_left2, band_center2, band_right2, buf2);

		    rc3 = local_band_align(seqi, seqj3, len, len23, mat,
			        best_score3, tiden_no3, alnln3, distance3, talign_info3, 
			        band_left3, band_center3, band_right3, buf3);
		}
		else{
			////////////////////////////////////////////////////////////////////
			rc = local_band_align(seqi, seqj, len, len2, mat,
		...
		len_eff12 = (options.global_identity == 0) ? alnln2 : (lens - talign_info2[4]);
		len_eff13 = (options.global_identity == 0) ? alnln3 : (lens - talign_info3[4]);
		
		////////////////////////////////////////////////////////
		tiden_pc = tiden_no / (float) len_eff1;
		tiden_pc2 = tiden_no2 / (float) len_eff12;
		tiden_pc3 = tiden_no3 / (float) len_eff13;
		if( options.useDistance ){
			if (distance > options.distance_thd ) continue;
			if (distance >= seq->distance) continue; // existing distance
		}else{
			if (tiden_pc < options.cluster_thd || tiden_pc2 < options.cluster_thd || tiden_pc3 < options.cluster_thd ) continue;
...
int SequenceDB::CheckOneEST( Sequence *seq, WordTable & table, WorkingParam & param, WorkingBuffer & buf, const Options & options )
{
	...
	for(comp=0; comp<2; comp++){
		...
		for(; ic->count; ic++){
			...
			tiden_pc = tiden_no / (float)len_eff1;
			//printf( "%i %i\n", tiden_no, options.error_rate100 );
			if( options.useDistance ){
				if (distance > options.distance_thd ) continue;
...
void SequenceDB::ComputeDistance( const Options & options )
{
	...
	std::string output = strcat(aaa2 , ".dist");
...
void SequenceDB::DoClustering1( const Options & options )
{
	int i;
	int NAA = options.NAA;
	double aa1_cutoff = options.cluster_thd;
	...
	WorkingParam param( aa1_cutoff, aas_cutoff, aan_cutoff );//设置参数
	...
	for(i=0; i<N; ){
		...
		if( m > N ) m = N;
		printf( "\rpregrouping sequences from  %9i  to  %9i\n", i, m );
		fflush( stdout );
		for(int ks=i; ks<m; ks++){
			Sequence *seq = sequences[ks];
			i = ks + 1;
			...
			//printf( "comparing the %9d sequences：  %s\n", ks, seq->data );
...
void SequenceDB::ClusterTo( SequenceDB & other, const Options & options )
{
	...
	Vector<WorkingParam> params(T);
	Vector<WorkingParam> params2(T);
	Vector<WorkingParam> params3(T);
	Vector<WorkingBuffer> buffers(T);
	Vector<WorkingBuffer> buffers2(T);
	Vector<WorkingBuffer> buffers3(T);
	WorkingParam & param = params[0];
	WorkingParam & param2 = params[0];
	WorkingParam & param3 = params[0];
	WorkingBuffer & buffer = buffers[0];
	WorkingBuffer & buffer2 = buffers[0];
	WorkingBuffer & buffer3 = buffers[0];
	for(i=0; i<T; i++){
		params[i].Set( aa1_cutoff, aas_cutoff, aan_cutoff );
	...
	WordTable word_table( options.NAA, NAAN );
	WordTable word_table2( options.NAA, NAAN );
	WordTable word_table3( options.NAA, NAAN );
	...
	for(i=0; i<N; ){
		...
		if( T > 1 ){
			...
			for(int j=0; j<JM; j++){
				...
				int flag = other.CheckOne( seq, word_table,word_table2,word_table3, params[tid],params2[tid],params3[tid], buffers[tid],buffers2[tid],buffers3[tid], options );
				if ((flag == 1) || (flag == -1)) { // if similar to old one delete it
					if (! options.cluster_best) {
						seq->Clear();
						seq->state |= IS_REDUNDANT ;
			...
			for(int j=0; j<M; j++){
				...
				flag = other.CheckOne( seq, word_table,word_table2,word_table3, param,param2,param3, buffer,buffer2,buffer3, options );
				if ((flag == 1) || (flag == -1)) { // if similar to old one delete it
					if (! options.cluster_best) {
						seq->Clear();
						seq->state |= IS_REDUNDANT ;
		...
		printf( "\r..........%9i  compared  %9i  groups\n", i, NR2_red_no );
		word_table.Clear();
		word_table.size = 0;
		i = m;
	}
	...
	cout << endl;
	cout << sequences.size() << " compared\t" << NR2_red_no << " groups" << endl;
	temp_files.Clear();
}
